...
class Matrix:
    ...
    def __add__(self, other_matrix):
        ...
        add_matrix = [[0 for _ in range(count_columns)] for _ in range(count_rows)]
    ...
    def transposition(self):
        count_rows = len(self.matrix)
        count_columns = len(self.matrix[0])
        transposed = [[0 for _ in range(count_rows)] for _ in range(count_columns)]
    ...
    def __mul__(self, other_matrix):
        ...
        multi_matrix = [
            [0 for _ in range(count_columns_right)] for _ in range(count_rows_left)
        ]
        for m in range(count_rows_left):
            for n in range(count_columns_right):
                for o in range(count_columns_left):
                    multi_matrix[m][n] += self.matrix[m][o] * other_matrix.matrix[o][n]
...
class Vector:
    ...
    def __add__(self, other_vector):
        if len(self.vec) != len(other_vector.vec):
            print("Vectors cannot be folded")
            return
        new_vec = [0 for _ in range(len(self.vec))]
        for i in range(len(self.vec)):
            new_vec[i] = self.vec[i] + other_vector.vec[i]

        return Vector(new_vec)

    def __mul__(self, other_vector) -> float:
        if len(self.vec) != len(other_vector.vec):
